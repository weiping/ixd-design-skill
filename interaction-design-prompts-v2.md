...
完成色彩系统（一）和字体系统（二）后，必须将确定的 hex 值和字体族名称**回填**至 `doc/ixd/phase5-components.md`，替换所有 `TBD（→ 阶段六）` 占位符：
...
- 视觉样例板 HTML（可选）
- 深色模式对照表
- 回填完成的 `doc/ixd/phase5-components.md`（Token 值完整版）
...
所有批次完成后：
1. 读取阶段二 `doc/ixd/phase2-architecture.md` 页面清单
2. 扫描原型代码确认已实现的页面
3. 对比清单，标识遗漏页面
4. 如有遗漏，实现遗漏页面
...
**重要**：本章必须包含阶段四的完整交互规格内容，非摘要。
从 `doc/ixd/phase4-page-specs/` 复制每个页面的完整内容。

每页包含 9 个章节（不含走查）：
1. 概述 - 页面完整产品背景
2. 布局结构 - 完整网格/布局细节
3. 组件列表 - 所有元素及位置
4. 交互规格 - 所有手势、流程、过渡（完整细节）
5. 页面状态 - 7 种状态完整描述
6. 动效规格 - 时间、缓动曲线（完整数值）
7. 数据策略 - Mock 数据结构、加载状态
8. 适配规则 - 响应式、深色模式、无障碍规则
9. 微交互 - 所有微交互描述

- 跨平台产品：每个页面需同时描述 **移动端布局**（单栏）和 **桌面端布局**（多面板分栏），并标注两端差异点
...
4. **页面交互说明**：根据穷举版页面清单，全部页面的完整交互说明（布局/组件/交互行为/状态/动效）
